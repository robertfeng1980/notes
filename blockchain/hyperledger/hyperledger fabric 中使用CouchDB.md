...
在本教程中，将使用`Marbles`示例作为用例来演示如何将`CouchDB`与`Fabric`配合使用，并将`Marbles`部署到构建第一个网络（`BYFN`）教程网络。应该已完成安装示例代码、二进制文件和`Docker`镜像的任务。但是，运行`BYFN`教程不是本教程的先决条件，而是在本教程中提供必要的命令以使用网络。
