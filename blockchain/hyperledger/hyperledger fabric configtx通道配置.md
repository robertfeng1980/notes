...
`Hyperledger Fabric`区块链网络的**共享配置存储在集合配置交易**中，每个通道一个。每个配置交易通常由较短的名称`configtx`引用。
...
**配置作为`HeaderType_CONFIG`类型的交易存储在没有其他交易的块中**。这些块称为**配置块**，其中第一个称为**创世块**。
...
`signature_header`是为标准交易定义的，而签名是通过`ConfigUpdateEnvelope`消息中的`signature_header`字节和`config_update`字节的串联。
...
收到`CONFIG_UPDATE`后，定序人通过执行以下操作来**计算生成的`CONFIG`**：
...
7. 将新的`ConfigEnvelope`写入`CONFIG`类型的`Envelope`，并最终将其作为**新配置块中的唯一交易写入**。
...
定序系统通道需要定义**定序参数**，以及用于创建通道的**联合体**。定序服务**必须只有一个定序系统通道**，**它是第一个要创建的通道**（或更准确地引导）。建议**永远不要在定序系统通道创建配置中定义<u>应用程序</u>部分**，但可以进行测试。请注意，对定序系统通道具有**读访问权限的任何成员都可以看到所有通道创建**，因此应**限制此通道的访问权限**。
...
参与定序的每个组织**在`Orderer`组下都有一个组元素**。该组定义**单个参数`MSP`**，其中包含该组织的**加密身份信息**。`Orderer`组的值确定**排序节点的运行方式**。它们存在于**每个通道**中，因此`orderer.BatchTimeout`例如可以在一个通道上以不同方式指定。

在启动时，定序人面对的文件系统包含**许多通道**的信息。定序人通过识别定义了联合体组的**通道来识别系统通道**。联盟具有以下结构。
...
请注意，每个联盟都定义了**一组成员**，就像**定序组织的组织成员一样**。每个联盟还定义了`ChannelCreationPolicy`。这是一个用于**授权通道创建请求的策略**。通常，此值将设置为`ImplicitMetaPolicy`，**要求通道的新成员签名以授权创建通道**。

# 应用程序通道配置

**应用程序配置**适用于为**应用程序类型交易设计的通道**。它的定义如下：
...
**应用程序通道对定序人组织和共识选项的副本进行编码**，以允许确定性地更新这些参数，因此包括来自定序人系统通道配置的相同`Orderer`部分。但是从应用程序的角度来看，这可能会被忽略。

# 通道创建

当定序人收到**不存在**的通道的`CONFIG_UPDATE`时，定序人会认为这必须是**通道创建请求**并执行以下操作。

1. 定序人**识别**要为其执行通道创建请求的**联盟**。它通过**查看顶级组的`Consortium`值**来实现。
2. 定序者**验证应用程序组**中包含的组织是相应**联盟中包含的组织的子集**，并且`ApplicationGroup`设置为`version 1`。
3. 定序人验证**如果联盟有成员，新通道也有申请成员**（创建联盟和没有成员的通道仅对测试有用）。
4. `orderer`通过从**定序系统通道获取`Orderer`组**，并使用**新指定的成员创建`Application`组**并将**其`mod_policy`指定为`consortium`配置中指定的`ChannelCreationPolicy`来创建模板配置**。请注意，**策略是在新配置的上下文中进行评估的**，因此需要**所有成员的策略**需要来自所有**新通道成员的签名**，而**不是联盟的所有成员**。
5. 然后，**定序人将`CONFIG_UPDATE`应用为此模板配置的更新**。**由于`CONFIG_UPDATE`对`Application` 组应用了修改**（其`version` 为`1`），因此配置代码会根据`ChannelCreationPolicy`验证**这些更新**。如果通道创建包含**任何其他修改**（例如，对于单个组织的锚点对等体），**则将调用该元素的相应`mod`策略**。
6. 具有**新通道配置的新`CONFIG`交易**被**包装**并**发送在定序系统通道上进行定序**。定序后，将创建该通道。