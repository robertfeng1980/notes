...
+ **全局**检查站有效性政策（`GCVP`）。可以**全局指定检查点有效性策略**。这类似于**本地对等策略**，除了它是在**系统（区块链）粒度**而不是对等粒度上规定的。例如，`GCVP`可以指定：
  + 如果由`11`个不同的对等方确认，则每个对等方可以信任检查点。
  + 在特定部署中，**每个订购者**与同一台机器（即信任域）中的**对等方并置**，并且最多`f`订购者可能（拜占庭）有故障，如果由`f + 1`个**不同的对等方确认**，则**每个对等方可以信任检查点与订货人并列**。