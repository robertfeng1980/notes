...
可以**通过链代码（以及`Hyperledger Fabric`运行时）基于身份的属性来做出访问控制决策**。这称为基于属性的访问控制，简称`ABAC`。

为了实现这一点，身份的注册证书（`ECert`）可以包含一个或多个属性名称和值。然后，链码提取属性的值以进行访问控制决策。例如，假设正在开发应用程序`app1`并希望只有`app1`管理员才能访问特定的链代码操作。链代码可以验证调用者的证书（由`CA`信任的`CA`颁发）包含名为`app1Admin`且值为`true`的属性。当然，属性的名称可以是任何值，值不必是布尔值。
