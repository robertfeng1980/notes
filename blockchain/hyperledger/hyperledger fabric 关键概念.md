...
**本地MSP仅在**它们应用**的节点或用户的文件系统上定义**。因此，在物理上和逻辑上，每个节点或用户只有一个本地`MSP`。但是，由于通道`MSP`可用于通道中的所有节点，因此它们在通道配置中进行逻辑定义一次。但是， **通道MSP也在通道中每个节点的文件系统上实例化，并通过一致性保持同步**。因此，虽然每个节点的本地文件系统上存在每个信道`MSP`的副本，但逻辑上`MSP`通道驻留在该通道上并由通道或网络维护。
