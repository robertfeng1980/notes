...
正如我们所见，**交易记录了世界状态**的变化。让我们看一下包含块中事务的详细**`blockdata`**结构。 

![ledger.transaction](https://hyperledger-fabric.readthedocs.io/en/latest/_images/ledger.diagram.5.png)

上图表示如下：块`B1`的块数据`D1`中的事务`T4`包括事务头`H4`，事务签名`S4`，事务提议`P4`，事务响应`R4`和认可列表`E4`。

在上面的示例中，我们可以看到以下字段：

- `header` **头**：（由`H4`说明）捕获有关事务的一些基本元数据， 例如，**相关链码的名称及其版本**。
- `Signature` **签名**：由`S4`说明的该部分包含由**客户端应用程序创建的加密签名**。此字段用于**检查事务详细信息是否未被篡改**，因为它需要应用程序的**私钥**来生成它。
- `Proposal`**提议**：由`P4`说明的该字段将应用程序提供的**输入参数**编码到链代码，该链代码创建提议的分类帐更新。当链码运行时，该**提议提供了一组输入参数**，这些**参数与当前世界状态**一起**确定新的世界状态**。
- `Response` **响应**：由`R4`说明的该部分捕获**世界状态的前后值**，作为**读写集**（`RW`集）。它是**链代码的输出**，如果事务成功验证，它将应用于**分类帐以更新世界状态**。
...
当分类帐状态结构化为`JSON`文档时，`CouchDB`是一个特别合适的选择，因为`CouchDB`支持**丰富**的查询和更新业务事务中常见的**更丰富的数据类型**。在实现方面，`CouchDB`在**单独**的操作系统进程中运行，但网络节点和`CouchDB`实例之间仍然存在`1：1`的关系。所有这些对于链码都是**不可见**的。有关`CouchDB`的更多信息，请参见[`CouchDB`作为`StateDatabase`](https://hyperledger-fabric.readthedocs.io/en/latest/couchdb_as_state_database.html)。
...
上图表达如下：分类帐`L`包括世界状态`W`和区块链`B`。`W`包含四个具有键的状态：`CAR1，CAR2，CAR3`和`CAR4`。`B`包含两个块，`0`和`1`。块`1`包含四个事务：`T1，T2，T3，T4`。
...
我们还可以看到分类帐区块链包含两个块。块0是**创世块**，但它**不包含**任何与汽车相关的交易。然而，块1包含事务`T1，T2，T3，T4`，并且这些事务对应于在世界状态中为`CAR0`到`CAR3`创建**初始状态**的事务。我们可以看到块`1`链接到块`0`。
